...
export const ATTRIBUTE_FILTERS: FilterDefinition[] = [
  ...
  // Placed FIRST and named "Catalog" rather than "Import": it is the coarsest
  // scope on the screen — which loaded catalog you are looking at — and it was
  // not findable when it sat mid-list looking like every other dropdown. Both
  // loads share table-name prefixes and some table names, so this is the only
  // reliable split.
  { key: "importBatch", labelEn: "Catalog (import)", labelAr: "الكتالوج (الاستيراد)", group: "identity", control: "multiselect", optionsSource: "distinct" },
