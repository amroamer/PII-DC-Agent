...
export function composeSystemPrompt(
  ...
  const rules = [
    ...
    "6. BUSINESS / legal-entity identifiers are NOT personal data — return not_pii. A customs registration code, importer code / customs code, trade licence number, establishment or company registration number, tax/VAT number, or a company / trade name identify a BUSINESS (a legal entity), not a natural person. Apply this consistently across every table (do not flip a trade licence or customs code to personal in one table and not another). This is distinct from a natural person's OWN identifiers — their name, Emirates ID, passport, personal mobile/email, or date of birth — which stay personal.",
  ].join("\n");
